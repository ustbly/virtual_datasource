 ...
 */
public class FusionDataPublisherByGRPC_Fix {
    ...
    private void subscribeGrpcStream(long sourceId, String topicStr) {
        ManagedChannel channel = ManagedChannelBuilder.forAddress("192.168.1.40", 6161).usePlaintext().build();
...
//                        System.out.println("接收到测向站数据: " + survey.getResultFrom() + ", 信号: " + survey.getHopSignalListList());
...
//        System.out.println("Estimated position: " + est.lat + ", " + est.lon);
        ...
        if (errKm.abs().compareTo(tolerance) < 0) { //定位误差小于1m
            ...
            if (tgt.getCampValue() == 1) {
                // 清空已有的LinkEquipments
                builder.clearLinkEquipments();

                // 红方可选装备列表

                Target.LinkEquipment.Builder linkbuilder = Target.LinkEquipment.newBuilder()
                        .setName("VU");
                ...
                builder.setReliability(ThreadLocalRandom.current().nextInt(5) + 1)
                        ...
                        .addLinkEquipments(linkbuilder)
                        .setPurpose("RED CAMP");

                Set<String> types = collectEquipmentTypes(selected);
                configs.stream()
                        .filter(cfg -> types.contains(cfg.type.toUpperCase()))
                        // 过滤掉你不想添加的类型，比如"LINK4A"
                        .filter(cfg -> !cfg.type.equalsIgnoreCase("LINK4A"))
                        .forEach(cfg -> {
                            if (isLink(cfg.type)) {
