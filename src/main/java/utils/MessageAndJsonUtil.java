 ...
 */
public class MessageAndJsonUtil {
    // 注册支持的消息类型解析函数
    ...
    static {
        ...
        // 注册 NetworkList 类型（融合后网域数据）
        parserRegistry.put(NetworkDomain.NetworkList.getDescriptor().getFullName(), any -> {
            try {
                // 尝试将 Any 类型解析为 NetworkList
                NetworkDomain.NetworkList msg = any.unpack(NetworkDomain.NetworkList.class);
                return toJson(msg);
            } catch (InvalidProtocolBufferException e) {
                return "[Error] Failed to parse NetworkList: " + e.getMessage();
            }
        });
    }
